...
# ============================================================
# MolLogP computation (Wildman–Crippen)
# ============================================================
df_final["mollogp"] = df_final["mol"].apply(Descriptors.MolLogP)

# ============================================================
# visualize one molecular graph
# ============================================================
smile = df_final["smiles"][92]
g_vis = from_smiles(smile, with_hydrogen=False)
G = to_networkx(g_vis)
...
# ============================================================
# Graph dataset preparation (MolLogP added)
# ============================================================
graph_list = []
...
for i, smi in enumerate(df_final["smiles"]):
    ...
    # MolLogP as global descriptor (broadcast to nodes)
    mol_logp = torch.tensor([df_final.loc[i, "mollogp"]], dtype=torch.float)
    mol_logp = mol_logp.repeat(g.x.size(0), 1)
    g.x = torch.cat([g.x, mol_logp], dim=1)

    g.y = torch.tensor([df_final.loc[i, "exp"]], dtype=torch.float)
    graph_list.append(g)

# ============================================================
# Print feature summary (important for reviewers)
# ============================================================
print("\n===== Feature Summary =====")
...
model = AttentiveFP(in_channels=10, hidden_channels=192, out_channels=1,
                    edge_dim=3, num_layers=6, num_timesteps=2, dropout=0.2).to(device)
