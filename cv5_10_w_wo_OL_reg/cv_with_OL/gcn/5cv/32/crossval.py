...
for fold, (train_ids, test_ids) in enumerate(kfold.split(graph_list)):
    ...
    train_loader = DataLoader(graph_list, batch_size=64, sampler=SubsetRandomSampler(train_ids))
    test_loader = DataLoader(graph_list, batch_size=64, sampler=SubsetRandomSampler(test_ids))

    # Define model and optimizer
    model = GCN(in_channels=9, hidden_channels=256, out_channels=1,
                     num_layers=5, dropout=0.4).to(device)
